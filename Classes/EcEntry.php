...
class EcEntry
{
    ...
    {
        global $db;
        if (!$this->key && $this->values[$this->form->key]) {
            $this->key = $this->values[$this->form->key];
        }
    ...
    public function post() // add!
    {
        ...
        if (!$this->key) {
            $this->key = trim($this->values[$this->form->key]);
        }
        ...
        $qry = sprintf('SELECT e.DeviceID as deviceId, ev.value FROM entryvalue ev Join entry e on (ev.entry = e.idEntry) WHERE ev.projectName = \'%s\' AND ev.formName = \'%s\' AND ev.fieldName = \'%s\' AND ev.value = %s',
            $this->projectName, $this->formName, $this->form->keyField,
            $db->stringVal($this->values[$this->form->keyField]));
        //echo $qry;
        ...
        if (count($ents) > 0) {
            if (($this->form->survey->allowDownloadEdits || $ents[0]['deviceId'] == $this->deviceId) && $this->deviceId != 'web') {
                ...
                return sprintf('Message : Duplicate Key for %s > %s > %s', $this->formName, $this->form->keyField,
                    $this->values[$this->form->keyField]);
            }
        }
        ...
        for ($i = 0; $i < $length; ++$i) {
            if ($this->form->fields[$keys[$i]]) {
                if (($this->form->fields[$keys[$i]]->type == 'gps' || $this->form->fields[$keys[$i]]->type == 'location') && !is_string($this->values[$keys[$i]])) {
                    ...
                    array_push($ins, sprintf('( %s, \'%s\', \'%s\', \'%s\', %s, %s)',
                        $this->form->fields[$keys[$i]]->idField, $this->form->survey->name, $this->form->name,
                        $keys[$i], $db->stringVal($this->values[$keys[$i]]), $this->id));
                }
            } else {
                $res = $db->rollbackTransaction();
                //echo $res;
    ...
    public static function postEntries($entries)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            if ($prj->tables[$entries[$i]->formName]->checkExists($entries[$i]->values[$keyfield])) {
                throw new Exception(sprintf('Your data could not be uploaded, there was a duplicate key for entry %s on line %s of your CSV file',
                    $entries[$i]->values[$keyfield], $i + 2));
            }
            ...
            if (!array_key_exists($key, $ent->values)) {
                ...
                if (($this->form->fields[$key]->type == 'gps' || $this->form->fields[$key]->type == 'location') && !is_string($this->values[$key])) {
                    ...
                    $in = sprintf('( %s, \'%s\', \'%s\', \'%s\', %s, %u)',
                        $this->form->fields[$key]->idField, $this->form->survey->name, $this->form->name, $key,
                        $db->stringVal($this->values[$key]), $ent->id);
                }
                $qry .= $in;
                $res = $db->do_query($qry);
