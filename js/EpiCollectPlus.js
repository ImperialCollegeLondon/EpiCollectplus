...
EpiCollect.Validators = {
    ...
    "regex": function (value, params, callback, ctrl_id) {
        ...
        if (!(value.match(new RegExp(params.regex)))) {
            obj = {
                valid: false,
                messages: [params.regexMessage ? params.regexMessage : "The value you have entered is not in the right format."],
                name: 'regex',
                control_id: ctrl_id
            };
        }
    ...
    "verify": function (value, params, callback, ctrl_id) {
        ...
        var obj = {};
        EpiCollect.prompt({
            content: "Please re-enter the value for " + this.text + " to confirm the value", callback: function (newvalue) {
                if (newvalue !== value) {
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
                if (data.length > 0) {
                    callback({
                        valid: false,
                        messages: ["This field must be unique, the value " + value + " had already been saved for this form."],
                        name: 'key',
                        control_id: ctrl_id
                    });
                }
...
EpiCollect.LoadingOverlay = function () {
    ...
    // bdy.append('<div id="fakeLoader"></div>');
...
EpiCollect.Form = function () {
    ...
    this.displayForm = function (cnf)//(ele, data, vertical, index, editMode )
    {
        ...
        if (editMode) {
            this.formElement.addClass('editing');
        } else {
        ...
        if (this.gpsFlds.length > 0) $(".locationControl", this.formElement).each(function (idx, ele) {
            $(ele).gpsPicker();
        });
...
EpiCollect.Field = function () {
    ...
    this.parse = function (xml) {
        ...
        for (var o = 0; o < opts.length; o++) {
            if (!opts[o].getElementsByTagName('value')[0].firstChild) {
                throw 'Option ' + o + ' for field ' + this.id + ' does not have a value';
            }
            if (!opts[o].getElementsByTagName('label')[0].firstChild) {
                throw 'Option ' + o + ' for field ' + this.id + ' does not have a label';
            }
